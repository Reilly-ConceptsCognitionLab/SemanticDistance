#' viz_dists_continuous
#'
#' Produces time varying plots of semantic distance changes by kernel size (ngram, word, turn) at each
#' sequential timepoint. Annotated large distance jumps (z>2.5) with a red line.
#'
#' @name viz_dists_continuous
#' @param dat dataframe with cosine semantic dist values in columns (CosDist_) labeled by 'row_id_postsplit'(either monologue or dialogue)
#' @param lang_sample quoted argument 'monologue' or 'dialogue', default is monologue
...
viz_dists_continuous <- function(dat, lang_smaple = "monologue", interpolate=TRUE, roll_avg=0,
                        annotate=TRUE)  {

    # Select and reshape data
    dat <- dat %>% dplyr::select(id_row_postsplit, contains("CosDist_")) %>%
      tidyr::pivot_longer(cols = contains("CosDist_"),
    ...
    p <- ggplot2::ggplot(dat, ggplot2::aes(x = word_order, y = Cos_Dist, color = Distance_Type)) +
      ...
      ggplot2::facet_wrap(~Distance_Type, ncol=1)
