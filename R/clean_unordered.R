...
clean_unordered <- function(dat, wordcol, clean = TRUE, omit_stops = TRUE, lemmatize = TRUE, split_strings = TRUE) {
  # Input validation
  ...
  dat_prep <- dat_prep %>%
    dplyr::mutate(id_row_postsplit = seq_len(nrow(dat_prep))) %>%
    ...
    dplyr::ungroup() %>%
    dplyr::select(-is_stopword)  # Remove the temporary stopword flag
