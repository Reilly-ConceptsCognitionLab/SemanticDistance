...
viz_mono <- function(dat, interpolate=TRUE, roll_avg=0, facet=TRUE, annotate=TRUE) {
  ...
  dat <- dat %>%
    dplyr::select(id_row_postsplit, contains("CosDist_")) %>%
    tidyr::pivot_longer(
  ...
  if(interpolate) {
    dat <- dat %>%
      ...
      dplyr::mutate(Cos_Dist = zoo::na.approx(Cos_Dist, na.rm = FALSE)) %>%
      dplyr::ungroup()
  }
  ...
  dat <- dat %>%
    dplyr::group_by(Distance_Type) %>%
    dplyr::mutate(
      z_CosDist = scale(Cos_Dist)  # Compute z-scores
    ) %>%
    dplyr::ungroup()
  ...
  if(roll_avg > 0) {
    dat <- dat %>%
      dplyr::group_by(Distance_Type) %>%
      dplyr::mutate(
        Cos_Dist = zoo::rollmean(Cos_Dist, k = roll_avg, fill = NA, align = "center"),
  ...
  p <- ggplot2::ggplot(dat, aes(x = word_order, y = Cos_Dist, color = Distance_Type)) +
    ggplot2::geom_line() +
    scale_color_manual(values = degas_pal) +  # Apply Degas palette
    coord_cartesian(ylim = c(0, 1.5))  # show only visual range of cosdist not cutting off datapoints

  # Add vertical red lines for significant z-scores if annotate=TRUE
  if(annotate) {
    # Get points where z_CosDist > 2.5
    sig_points <- dat %>%
      dplyr::filter(z_CosDist > 2.5)
    ...
    # Add labels
    p <- p + ggplot2::labs(
      title = "Cosine Distance Trajectories",
      x = "Word Order",
      y = "Cosine Distance",
      color = "Distance Type"
    )
  ...
  if(facet) {
    p <- p + ggplot2::facet_wrap(~Distance_Type)
  }
